      ...
      >
        ...
        <div className="h-12 border-b border-border-primary bg-bg-tertiary flex items-center justify-between px-4 flex-shrink-0">
          <div className="flex items-center space-x-2">
            ...
            <span id="release-notes-modal-title" className="font-semibold text-sm text-text-primary">
              What's New in Boltt v{availableVersion || "Latest"}
            </span>
          </div>
          <button
            onClick={() => setNotesModalOpen(false)}
            className="text-text-secondary hover:text-text-primary p-1 rounded hover:bg-bg-hover transition cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-text-accent"
            title="Close"
          >
            <X size={16} />
          </button>
        ...
        <div className="p-5 flex flex-col space-y-4 overflow-y-auto flex-1">
          ...
          <div className="flex flex-col space-y-2">
            ...
            <div className="p-4 rounded-lg bg-bg-primary border border-border-primary text-xs text-text-primary font-mono whitespace-pre-wrap leading-relaxed max-h-[320px] overflow-y-auto select-text">
        ...
        <div className="h-12 border-t border-border-primary bg-bg-tertiary flex items-center justify-between px-4 flex-shrink-0">
          <button
            onClick={() => setNotesModalOpen(false)}
            className="px-3.5 py-1.5 border border-border-primary hover:bg-bg-hover rounded text-xs font-medium text-text-secondary hover:text-text-primary transition cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-text-accent"
          >
            Close
          </button>
          ...
          {status === "ready" && (
            <button
              onClick={() => relaunchApp()}
              className="px-4 py-1.5 bg-[#4ade80] hover:bg-[#22c55e] text-black rounded text-xs font-bold transition cursor-pointer flex items-center space-x-1.5 shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-1 focus-visible:ring-[#4ade80]"
            >
              <RotateCcw size={13} />
