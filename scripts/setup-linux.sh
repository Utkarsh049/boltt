...
if [ "$CHECK_ONLY" = true ]; then
  ...
  if [ -n "$PKG_MANAGER" ] && [ "$PKG_MANAGER" != "manual" ]; then
    ...
    if [ "$YES_FLAG" = true ]; then
      ...
      read -p "Install required Linux C/C++ libraries ($PKG_MANAGER)? [Y/n] " -n 1 -r
      echo ""
...
if command -v node >/dev/null 2>&1; then
  ...
  echo "Please install Node.js 18+ (e.g., via nvm: https://github.com/nvm-sh/nvm or package manager)."
fi
...
if command -v pnpm >/dev/null 2>&1; then
  ...
  if [ "$CHECK_ONLY" = false ]; then
    ...
    if [ "$YES_FLAG" = true ]; then
      ...
      read -p "Install pnpm globally via corepack / npm? [Y/n] " -n 1 -r
      echo ""
      if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        DO_INSTALL=true
      fi
